...
// Helper para upload no Cloudinary (Grátis e Robusto)
async function uploadToCloudinary(base64Data, settings) {
    // 1. Configura com as chaves do banco
    ...
    return new Promise((resolve, reject) => {
        ...
        // Converte base64 para buffer e envia
...
async function generatePost(settings) {
    ...
    if (imageProvider === 'imagen') {
        console.log("🎨 Gerando imagem com Google Imagen 3 (Storage no Cloudinary)...");
        try {
            ...
            if (response.candidates && response.candidates[0]?.content?.parts[0]?.inlineData) {
                ...
                console.log("✅ Imagem salva no Cloudinary:", finalImageUrl);
            } else {
                throw new Error("Formato de resposta do Imagen não suportado pela lib atual.");
            }

        } catch (imgError) {
            console.warn("⚠️ Falha no Imagen 3/Cloudinary (Fallback para Pollinations):", imgError.message);
        ...
        // Pollinations FLUX (Padrão - Não precisa de Storage)
