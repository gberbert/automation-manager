        ...
        <div className="space-y-8 pb-20">
            ...
            {activeTab === 'connection' && (
                <div className="bg-gray-800/50 backdrop-blur p-6 rounded-xl border border-gray-700 space-y-6 animate-fadeIn">
                    ...
                    <div className="pt-4 border-t border-gray-700"><button onClick={() => { const authUrl = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${settings.linkedinClientId}&redirect_uri=${encodeURIComponent(settings.linkedinRedirectUri)}&scope=openid%20profile%20email%20w_member_social%20r_member_social`; window.open(authUrl, 'LinkedIn OAuth', 'width=600,height=700'); }} className="w-full flex items-center justify-center space-x-2 bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-semibold transition-colors"><LinkedinIcon className="w-5 h-5" /><span>Connect LinkedIn Account (Re-authenticate)</span></button><p className="text-xs text-gray-400 mt-2 text-center text-yellow-400">⚠️ Se você já conectou antes, clique acima novamente para liberar permissões de leitura de comentários.</p></div>
