...
import { Save, Plus, Trash2, Linkedin as LinkedinIcon, MessageSquare, Key, Eye, EyeOff, BookOpen, Cpu, Image as ImageIcon, Layers, Loader2, FileText, Target, X, Edit2, Check, Calendar } from 'lucide-react';
...
export default function Linkedin() {
    ...
    const [settings, setSettings] = useState({
        ...
        // Mantemos 'source' como 'arxiv' por padrão para ativar a busca automática no backend
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    if (!data.strategyImage && data.promptTemplate) {
                        loadedImage = { template: data.promptTemplate, contexts: data.contexts || [], topics: data.topics || [] };
                    }
                    ...
                    setSettings({
                        ...
                        strategyPdf: { ...defaultPdf, ...data.strategyPdf },
                        lastUsedStrategy: data.lastUsedStrategy || 'pdf'
                    });
                }
            } catch (error) { console.error("Erro crítico:", error); setLoadError(true);
    ...
    const openModal = (type, index, value) => { setViewingItem({ type, index });
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="bg-gray-800/50 backdrop-blur p-6 rounded-xl border border-gray-700 space-y-4 flex-1">
                                ...
                                <div className="flex gap-2"><textarea rows={2} value={newContext} onChange={(e) => setNewContext(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-2 text-white text-xs focus:border-yellow-500 outline-none" placeholder="Novo contexto..." /><button onClick={() => addList('contexts', newContext, setNewContext)} className="bg-yellow-600 px-3 rounded text-white flex items-center"><Plus className="w-4 h-4" /></button></div>
                                    ...
                                    {(activeConfig?.contexts || []).map((ctx, index) => {
                                        ...
                                        return (
                                            <div key={index} className="flex justify-between bg-yellow-900/20 p-2 rounded border border-yellow-800/50">
                                                <p className="text-xs text-yellow-100 truncate w-40">
                                                    <span className="font-bold mr-2 text-yellow-500">#{index + 1}</span>{displayVal}
                                                </p>
