...
import { Save, Plus, Trash2, Linkedin as LinkedinIcon, MessageSquare, Key, Eye, EyeOff, BookOpen, Cpu, Image as ImageIcon, Layers, Loader2, FileText, Target, X, Edit2, Check, Calendar, Palette } from 'lucide-react';
import LinkedinGuide from '../components/LinkedinGuide';
import { GEMINI_MODELS, IMAGE_PROVIDERS } from '../constants';

export default function Linkedin() {
    const [activeTab, setActiveTab] = useState('connection');
    const [strategyTab, setStrategyTab] = useState('image'); 
    ...
    // Estados do Modal (Agora suporta Texto + Contexto de Imagem)
        ...
        const fetchSettings = async () => {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    // Helper para normalizar contextos (String -> Objeto)
                    // Isso garante compatibilidade com dados antigos
                    const normalizeContexts = (ctxList) => {
                        if (!Array.isArray(ctxList)) return [];
                        return ctxList.map(c => {
                            if (typeof c === 'string') return { text: c, imageContext: '' };
                            return c;
                        });
                    };

                    let loadedImage = data.strategyImage || defaultImage;
                    if (!data.strategyImage && data.promptTemplate) {
                        loadedImage = { 
                            template: data.promptTemplate, 
                            contexts: normalizeContexts(data.contexts), 
                            topics: data.topics || [] 
                        };
                    } else {
                        loadedImage.contexts = normalizeContexts(loadedImage.contexts);
                    }

                    let loadedPdf = { ...defaultPdf, ...data.strategyPdf };
                    loadedPdf.contexts = normalizeContexts(loadedPdf.contexts);
                    ...
                    setSettings({
                        ...
                        strategyImage: loadedImage,
                        strategyPdf: loadedPdf,
                        lastUsedStrategy: data.lastUsedStrategy || 'pdf'
                    });
                }
            } catch (error) {
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            // Limpa campos legados para não sujar o banco com duplicatas
            const cleanSettings = { ...settings, promptTemplate: null, contexts: null, topics: null, postFormat: null, pdfStrategy: null };
    ...
    const updateStrategy = (field, value) => {
        const target = strategyTab === 'image' ? 'strategyImage' : 'strategyPdf';
        setSettings(prev => ({
            ...prev,
    ...
    const addList = (field, value, setFn) => {
        if (value.trim()) {
            ...
            // Se for contexto, cria estrutura de objeto
            if (field === 'contexts') {
                newItem = { text: value.trim(), imageContext: '' };
            }

            updateStrategy(field, [...currentList, newItem]);
            setFn('');
        }
    };
    ...
    const handleSaveModal = () => {
        ...
        const targetStrategyName = strategyTab === 'image' ? 'strategyImage' : 'strategyPdf';
        const field = viewingItem.type;
        const newList = [...settings[targetStrategyName][field]];
        
        if (field === 'contexts') {
            // Salva texto e contexto de imagem juntos
            newList[viewingItem.index] = {
                text: modalValue,
                imageContext: modalImageContext
            };
        } else {
            // Tópicos continuam sendo strings simples
    ...
    const openModal = (type, index, item) => { 
        setViewingItem({ type, index });
        
        if (type === 'contexts') {
            // Suporta legado (string) ou novo (objeto)
            const textVal = typeof item === 'string' ? item : item.text;
            const imgVal = typeof item === 'string' ? '' : (item.imageContext || '');
            setModalValue(textVal);
            setModalImageContext(imgVal);
        } else {
            setModalValue(item);
            setModalImageContext('');
        }
        
        setIsEditingModal(false); // Começa em modo leitura
    };

    const activeConfig = strategyTab === 'image' ? settings.strategyImage : settings.strategyPdf;
    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            <div className="flex space-x-4 border-b border-gray-700 overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('prompt')} className={`pb-4 px-4 font-medium transition-colors ${activeTab === 'prompt' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-gray-400'}`}><div className="flex items-center gap-2"><Layers className="w-4 h-4" /> Strategies (A/B)</div></button>
                    ...
                    <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="p-4 border-b border-gray-700 flex justify-between items-center bg-gray-800/50 rounded-t-xl">
                            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                ...
                                Editando {viewingItem.type === 'contexts' ? 'Contexto' : 'Tópico'} #{viewingItem.index + 1}
                            </h3>
                            <button onClick={() => setViewingItem(null)} className="text-gray-400 hover:text-white"><X className="w-6 h-6" /></button>
                        </div>
                        
                        <div className="p-6 overflow-y-auto bg-gray-900 flex-1 space-y-6">
                            {/* Campo Principal (Texto do Contexto ou Tópico) */}
                            <div>
                                <label className="text-sm font-semibold text-gray-400 mb-2 block">
                                    {viewingItem.type === 'contexts' ? 'Texto do Contexto (Personalidade/Tom):' : 'Descrição do Tópico:'}
                                </label>
                                {isEditingModal ? (
                                    <textarea 
                                        className="w-full h-40 bg-gray-800 text-white p-4 rounded-lg border border-blue-500/50 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm leading-relaxed resize-none"
                                        value={modalValue}
                                        onChange={(e) => setModalValue(e.target.value)}
                                    />
                                ) : (
                                    <p className="text-gray-300 whitespace-pre-wrap font-mono text-sm leading-relaxed p-4 bg-gray-800/30 rounded-lg border border-gray-700">
                                        {modalValue}
                                    </p>
                                )}
                            </div>

                            {/* Campo Extra: Detalhes da Imagem (Apenas para Contextos) */}
                            {viewingItem.type === 'contexts' && (
                                <div className="pt-4 border-t border-gray-700">
                                    <label className="text-sm font-semibold text-green-400 mb-2 flex items-center gap-2">
                                        <Palette className="w-4 h-4" /> Detalhes da Imagem (Vinculado a este contexto):
                                    </label>
                                    <p className="text-xs text-gray-500 mb-2">
                                        Instruções visuais específicas que serão SEMPRE usadas quando este contexto for sorteado (ex: Logos, Cores, Estilo).
                                    </p>
                                    {isEditingModal ? (
                                        <textarea 
                                            className="w-full h-24 bg-gray-800 text-green-100 p-4 rounded-lg border border-green-500/30 focus:outline-none focus:ring-2 focus:ring-green-500 font-mono text-sm leading-relaxed resize-none"
                                            value={modalImageContext}
                                            onChange={(e) => setModalImageContext(e.target.value)}
                                            placeholder="Ex: Inclua o logo da NTT DATA no canto superior direito. Estilo corporate clean."
                                        />
                                    ) : (
                                        <p className="text-green-300/80 whitespace-pre-wrap font-mono text-sm leading-relaxed p-4 bg-green-900/10 rounded-lg border border-green-900/30 min-h-[60px]">
                                            {modalImageContext || "(Nenhum detalhe de imagem configurado)"}
                                        </p>
                                    )}
                                </div>
                            )}
                        ...
                        <div className="p-4 border-t border-gray-700 bg-gray-800/30 rounded-b-xl flex justify-end gap-3">
                            {!isEditingModal ? (
                                <>
                                    <button onClick={() => setIsEditingModal(true)} className="flex items-center gap-2 bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 px-4 py-2 rounded-lg border border-blue-500/30 transition-colors">
                                        <Edit2 className="w-4 h-4" /> EDITAR
                                    </button>
                                    <button onClick={() => setViewingItem(null)} className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">
                                        Fechar
                                    </button>
                                </>
                            ) : (
                                <>
                                    <button onClick={() => setIsEditingModal(false)} className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">
                                        Cancelar
                                    </button>
                                    <button onClick={handleSaveModal} className="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded-lg font-bold shadow-lg transition-colors">
            ...
            {activeTab === 'prompt' && (
                <div className="space-y-6 animate-fadeIn">
                    <div className="flex bg-gray-800 p-1 rounded-lg">
                        <button onClick={() => setStrategyTab('image')} className={`flex-1 py-2 px-4 rounded flex gap-2 justify-center items-center text-sm font-medium transition-all ${strategyTab === 'image' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-gray-700'}`}>
                            <ImageIcon className="w-4 h-4" /> Texto + Imagem
                        </button>
                        <button onClick={() => setStrategyTab('pdf')} className={`flex-1 py-2 px-4 rounded flex gap-2 justify-center items-center text-sm font-medium transition-all ${strategyTab === 'pdf' ? 'bg-red-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-gray-700'}`}>
                            <FileText className="w-4 h-4" /> Texto + PDF (ArXiv)
                        </button>
                    </div>
                    ...
                    {strategyTab === 'pdf' && (
                        <div className="bg-red-900/20 border border-red-500/30 p-4 rounded-xl flex flex-wrap items-center gap-4">
                            ...
                            <div className="flex-1">
                                <h3 className="text-red-400 font-bold">Modo Curadoria de PDF</h3>
                                <p className="text-xs text-gray-400">O sistema buscará um artigo no ArXiv baseado no tópico e usará o resumo para criar o post.</p>
                            </div>
                            {/* NOVO CAMPO: FILTRO DE ANO */}
                            <div className="flex items-center gap-2">
                                <label className="text-xs text-red-300 flex items-center gap-1"><Calendar className="w-3 h-3"/> Ano Mín:</label>
                                <input 
                                    type="number" 
                                    min="2020" max="2030" 
                                    value={settings.strategyPdf?.dateFilter || '2024'} 
                                    onChange={(e) => updateStrategy('dateFilter', e.target.value)}
                                    className="bg-gray-900 text-white px-3 py-1 rounded border border-gray-600 text-sm w-20 text-center"
                                />
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-gray-800/50 backdrop-blur p-6 rounded-xl border border-gray-700 space-y-4">
                            <h3 className={`text-xl font-semibold flex items-center gap-2 ${strategyTab === 'image' ? 'text-blue-400' : 'text-red-400'}`}>
                                <MessageSquare className="w-5 h-5" /> Prompt Base ({strategyTab.toUpperCase()})
                            </h3>
                            <textarea rows={12} value={activeConfig?.template || ''} onChange={(e) => updateStrategy('template', e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 text-white focus:border-blue-500 outline-none font-mono text-sm" placeholder="Digite o prompt..." />
                        </div>

                        <div className="flex flex-col gap-6">
                            {/* Contexts Pool */}
                            <div className="bg-gray-800/50 backdrop-blur p-6 rounded-xl border border-gray-700 space-y-4 flex-1">
                                <h3 className="text-xl font-semibold text-yellow-400 flex items-center gap-2"><Target className="w-5 h-5" /> Contexts Pool</h3>
                                <div className="flex gap-2">
                                    <textarea rows={2} value={newContext} onChange={(e) => setNewContext(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-2 text-white text-xs focus:border-yellow-500 outline-none" placeholder="Novo contexto..." />
                                    <button onClick={() => addList('contexts', newContext, setNewContext)} className="bg-yellow-600 px-3 rounded text-white flex items-center"><Plus className="w-4 h-4" /></button>
                                </div>
                                <div className="flex flex-col gap-2 max-h-40 overflow-y-auto">
                                    {(activeConfig?.contexts || []).map((ctx, index) => {
                                        const displayText = typeof ctx === 'string' ? ctx : ctx.text;
                                        // Verifica se tem contexto de imagem
                                        const hasImageCtx = typeof ctx === 'object' && ctx.imageContext && ctx.imageContext.trim() !== '';

                                        return (
                                            <div key={index} className="flex justify-between bg-yellow-900/20 p-2 rounded border border-yellow-800/50">
                                                <div className="flex items-center gap-2 overflow-hidden">
                                                    <span className="font-bold text-yellow-500 text-xs">#{index + 1}</span>
                                                    <p className="text-xs text-yellow-100 truncate w-32">{displayText}</p>
                                                    {/* Ícone indicador de imagem */}
                                                    {hasImageCtx && <Palette className="w-3 h-3 text-green-400 flex-shrink-0" title="Contexto de Imagem Vinculado" />}
                                                </div>
                                                <div className="flex gap-1">
                                                    <button onClick={() => openModal('contexts', index, ctx)} className="text-blue-400 hover:text-blue-300"><Eye className="w-3 h-3"/></button>
                                                    <button onClick={() => removeList('contexts', index)} className="text-red-400 hover:text-red-300"><Trash2 className="w-3 h-3"/></button>
                                                </div>
                                            </div>
                                        );
                                    })}
                            ...
                            {/* Topics Pool */}
                            <div className="bg-gray-800/50 backdrop-blur p-6 rounded-xl border border-gray-700 space-y-4 flex-1">
                                <h3 className={`text-xl font-semibold flex items-center gap-2 ${strategyTab === 'image' ? 'text-blue-400' : 'text-red-400'}`}><Layers className="w-5 h-5" /> Topics Pool</h3>
                                <div className="flex gap-2"><input type="text" value={newTopic} onChange={(e) => setNewTopic(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && addList('topics', newTopic, setNewTopic)} className="flex-1 bg-gray-900 border border-gray-700 rounded p-2 text-white text-sm" placeholder="Novo tópico..." /><button onClick={() => addList('topics', newTopic, setNewTopic)} className={`${strategyTab === 'image' ? 'bg-blue-600' : 'bg-red-600'} px-3 rounded text-white`}><Plus className="w-4 h-4" /></button></div>
                                <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto">
                                    {(activeConfig?.topics || []).map((topic, index) => (
                                        <div key={index} className={`flex items-center gap-1 px-2 py-1 rounded border text-xs ${strategyTab === 'image' ? 'bg-blue-900/30 border-blue-800 text-blue-200' : 'bg-red-900/30 border-red-800 text-red-200'}`}>
